...
The runtime **SHALL** depend on **Click**, **Pillow**, **NumPy**, **opencv-python-headless**, **requests**, **PyTorch**, and **ultralytics** (default human detection: **YOLOv8m** COCO weights via `YOLO("yolov8m.pt")`) with lower bounds as declared in `[project.dependencies]` in `pyproject.toml`.
